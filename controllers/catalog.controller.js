...
exports.createCategory = async (req, res) => {
  ...
  if (getIsMongoConnected()) {
    try {
      ...
      console.error('[Category DB Sync Error]:', err.message);
    }
  }
...
exports.updateProduct = async (req, res) => {
  ...
  if (getIsMongoConnected()) {
    try {
      const ProductModel = require('../models/Product.model');
      await ProductModel.findOneAndUpdate({ id: updated.id }, updated, { upsert: true, new: true });
    } catch (err) {
      console.error('[Product DB Sync Error]:', err.message);
    }
  }
...
exports.createPriceSheet = async (req, res) => {
  const priceSheets = getTenantPriceSheets(req.tenantStore);
  ...
  if (getIsMongoConnected()) {
    try {
      const PriceSheetModel = require('../models/PriceSheet.model');
      await PriceSheetModel.findOneAndUpdate({ id: sheet.id }, sheet, { upsert: true, new: true });
    } catch (err) {
      console.error('[PriceSheet DB Sync Error]:', err.message);
    }
  }
...
exports.updatePriceSheet = async (req, res) => {
  const priceSheets = getTenantPriceSheets(req.tenantStore);
...
exports.deletePriceSheet = async (req, res) => {
  const priceSheets = getTenantPriceSheets(req.tenantStore);
  ...
  if (getIsMongoConnected()) {
    try {
      const PriceSheetModel = require('../models/PriceSheet.model');
      await PriceSheetModel.findOneAndDelete({ id: req.params.id });
    } catch (err) {
      console.error('[PriceSheet DB Sync Error]:', err.message);
    }
  }
...
exports.updatePriceSheetGrid = async (req, res) => {
  const store = req.tenantStore;
